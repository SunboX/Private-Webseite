...
var MilkboxDisplay= new Class({
	...
	build_html:function(){
		...
		this.caption = new Element('div#mbox-caption').setStyle('display','none');
    	this.count = new Element('div#mbox-count');

		this.bottom.adopt(this.caption, this.count, this.controls, new Element('div.mbox-reset'));
		
		this.close = new Element('a#mbox-close').set('text', 'Schließen');
		this.next = new Element('a#mbox-next').set('text', 'Vor →');
		this.prev = new Element('a#mbox-prev').set('text', '← Zurück');
		this.playpause = new Element('a#mbox-playpause');
		...
		this.controls.adopt(this.prev, new Element('span[text=" | "]'), this.next, new Element('span[text=" | "]'), this.playpause, this.close);
	},
	...
	show_file:function(file,caption,index,length){
		...
		this.caption.setStyle('width',final_w-caption_adds.paddingRight.toInt()-caption_adds.marginRight.toInt()-20);
		var mainbox_size = this.mainbox.getComputedSize();

		var final_h = file_size.h+filebox_addsize+this.bottom.getComputedSize().totalHeight + 47;
	...
	set_mode:function(gallery_type){
		...
		switch(gallery_type){
			case 'autoplay':
				$$(this.playpause,this.close,this.next,this.prev,this.count).setStyle('display','');
				break;
			case 'single':
				$$(this.playpause,this.next,this.prev,this.count).setStyle('display','none');
				break;
			case 'standard':
				$$(this.close,this.next,this.prev,this.count).setStyle('display','');
	...
	update_count:function(index,length){
		this.count.set('text','Bild ' + index+' von '+length);
	},
